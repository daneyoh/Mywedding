...
<section className="py-24 px-4 bg-zinc-50/50">
        ...
        <div className="grid grid-cols-3 gap-1">
          {/* 1번부터 24번까지 총 24장 이미지 렌더링 */}
          {[...Array(24)].map((_, i) => (
            <div 
              key={i} 
              className="aspect-square bg-stone-100 overflow-hidden cursor-pointer"
              onClick={() => setLightboxIndex(i+1)} // 클릭 시 1~24 저장
            >
              <img 
                src={`/Gallery ${i+1}.jpg`} 
                className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" 
                alt={`Gallery ${i+1}`} 
                onError={(e) => { e.currentTarget.src = `/Gallery ${i+1} .jpg`; }} 
              />
            </div> 
          ))}
        </div>
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              // [수정됨] 1번 사진에서 왼쪽을 누르면 24번 사진으로 이동
              setLightboxIndex(lightboxIndex === 1 ? 24 : lightboxIndex - 1);
            }}
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              // [수정됨] 24번 사진에서 오른쪽을 누르면 1번 사진으로 이동
              setLightboxIndex(lightboxIndex === 24 ? 1 : lightboxIndex + 1);
            }}
